...
Deno.serve(async (request) => {
  ...
  try {
    const { officeId, turnstileToken } = await request.json();
    if (
      typeof officeId !== "string" ||
      !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(officeId) ||
    ...
    if (
      ...
      verificationResult.action !== "reveal_contact" ||
    ...
    if (!office) {
      return new Response(JSON.stringify({ error: "Office not found" }), {
        status: 404,
        headers: { ...jsonHeaders, ...cors },
      });
    }
    ...
    console.error(error);
    return new Response(JSON.stringify({ error: "Unable to reveal contact" }), {
      status: 400,
      headers: { ...jsonHeaders, ...cors },
    });
  }
